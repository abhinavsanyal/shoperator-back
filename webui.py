...
async def run_org_agent(
    ...
    try:
        ...
        if status_callback:
            status_callback(final_result, errors, model_actions, model_thoughts, trace_file.get('.zip'), history_file)
...
async def run_custom_agent(
    ...
    try:
        ...
        # Get trace file only if path is provided
        trace_file = get_latest_files(save_trace_path).get('.zip') if save_trace_path else None

        if status_callback:
            # Extract step number from the history or use a default
            step_number = len(history.history) if history and history.history else 0
            
            # Call status callback with the correct number of arguments
            await status_callback(
                step_number=step_number,
                memory=final_result,  # Using final result as memory
                task_progress=model_thoughts,  # Using model thoughts as task progress
    ...
    if not headless:
        ...
        try:
            ...
            try:
                ...
                final_result, errors, model_actions, model_thoughts, latest_videos, trace, history_file, stop_button, run_button, browser_context = result
            except gr.Error:
                final_result = ""
                model_actions = ""
                model_thoughts = ""
